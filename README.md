...
## Known instability on some networks

While validating locally, the OrangeHRM demo's app shell occasionally hung indefinitely after the initial HTML loaded — no failed request, console error, or non-2xx response ever appeared (confirmed via the request/response/console listeners in `pages/LoginPage.ts`), which points to a connection being silently dropped somewhere on that network path (security software or a proxy doing SSL inspection) rather than a bug in the framework or the app itself. The same flow reliably succeeded on other runs and in isolation (`npx playwright open <url>`), and `--disable-blink-features=AutomationControlled` is set in `playwright.config.ts` as a mitigation for automation-detection-based interference. `retries: 2` is enabled everywhere (not just CI) as a pragmatic safety net for this specific class of failure. If a run stalls well past the configured timeouts, it's almost certainly this — the GitHub Actions CI run (clean, unrestricted network) is the more reliable place to confirm a genuine pass.
